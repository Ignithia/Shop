...
if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true) {
    try {
        ...
        if ($currentUser) {
            ...
            if (method_exists($currentUser, 'isBanned') && $currentUser->isBanned()) {
                $banReason = '';
                if (method_exists($currentUser, 'getBanReason')) {
                    $banReason = $currentUser->getBanReason();
                }
                // log out and redirect to login with ban info
                User::logout();
                $loginPath = ($__in_admin ? '../' : '') . 'login.php?ban=1&reason=' . rawurlencode($banReason);
