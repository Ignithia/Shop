...
class Game
{
    ...
    public function getReviews()
    {
        ...
        return Review::getByGame($this->pdo, $this->id);
    }
    ...
    public function addReview($userId, $text, $recommended)
    {
        ...
        $result = $review->create($userId, $this->id, $text, $recommended);
        if (is_array($result)) {
    ...
    public function getReviewStats()
    {
        ...
        return Review::getStatsForGame($this->pdo, $this->id);
    }
    ...
    public static function addCategory($pdo, $name, $description = '')
    {
        ...
        if (empty(trim($name))) {
            return ['success' => false, 'message' => 'Category name is required', 'category_id' => null];
        }
        if ($cat->create($name, $description)) {
            return ['success' => true, 'message' => 'Category added successfully', 'category_id' => $cat->getId()];
        }
        return ['success' => false, 'message' => 'Error adding category or already exists', 'category_id' => null];
    }
