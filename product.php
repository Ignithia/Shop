...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_review'])) {
    ...
    if ($review->loadById($review_id) && $review->getGameId() == $game_id) {
        if ($review->getUserId() == $currentUser->getId() || $currentUser->isAdmin()) {
            if ($review->delete()) {
    ...
    if (empty($review_text)) {
        ...
        if ($review->loadById($review_id) && $review->getGameId() == $game_id) {
            if ($review->getUserId() == $currentUser->getId()) {
                if ($review->update($review_text, $recommended)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
    if ($user_owns_game) {
        ...
        if (empty($review_text)) {
            ...
            $rev = new Review($pdo);
            $res = $rev->create($currentUser->getId(), $game_id, $review_text, $recommended);
            if (is_array($res)) {
                if (!empty($res['success'])) {
                    $review_message = $res['message'] ?? 'Review posted successfully!';
                } else {
                    $review_error = $res['message'] ?? 'Failed to post review.';
                }
            } else {
                if ($res) {
...
$reviews = Review::getByGame($pdo, $game_id);

// Calculate review statistics
$stats = Review::getStatsForGame($pdo, $game_id);
$total_reviews = $stats['total'] ?? 0;
$recommendation_percentage = $stats['percentage'] ?? 0;

// Check if current user has already reviewed
$user_has_reviewed = false;
if ($user_owns_game) {
    $user_has_reviewed = Review::existsByUserGame($pdo, $currentUser->getId(), $game_id);
}
